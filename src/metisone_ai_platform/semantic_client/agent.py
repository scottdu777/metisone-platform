...
class RemoteSemanticEditCommand:
    operation: Operation
    cube: str
...
class LocalSemanticEditAgent:
    """Client-side MVP agent that calls the remote edit service API."""

    def __init__(self, client: SemanticEditServiceClient) -> None:
        self.client = client

    def handle(self, message: str) -> tuple[RemoteSemanticEditCommand, dict[str, Any]]:
        command = self.parse(message)
        result = self.execute(command)
        return command, result

    def parse(self, message: str) -> RemoteSemanticEditCommand:
        text = message.strip()
        lowered = text.lower()
        ...
        return RemoteSemanticEditCommand(
            operation=operation,
            cube=cube,
    ...
    def execute(self, command: RemoteSemanticEditCommand) -> dict[str, Any]:
    ...
    def _execute_measure(self, command: RemoteSemanticEditCommand) -> dict[str, Any]:
        if command.operation == "create":
            assert command.sql is not None
            assert command.member_type is not None
            return self.client.create_measure(
                command.cube,
                command.member_name,
                command.sql,
                command.member_type,
                extra_fields=command.fields,
            )
        if command.operation == "modify":
            return self.client.modify_measure(
                command.cube,
                command.member_name,
                command.fields,
            )
        return self.client.delete_measure(command.cube, command.member_name)

    def _execute_dimension(self, command: RemoteSemanticEditCommand) -> dict[str, Any]:
        if command.operation == "create":
            assert command.sql is not None
            assert command.member_type is not None
            return self.client.create_dimension(
                command.cube,
                command.member_name,
                command.sql,
                command.member_type,
        ...
        if command.operation == "modify":
            return self.client.modify_dimension(
                command.cube,
                command.member_name,
                command.fields,
            )
        return self.client.delete_dimension(command.cube, command.member_name)

    def _execute_join(self, command: RemoteSemanticEditCommand) -> dict[str, Any]:
        if command.operation == "create":
            assert command.sql is not None
            assert command.relationship is not None
            return self.client.create_join(
                command.cube,
                command.member_name,
                command.sql,
        ...
        if command.operation == "modify":
            return self.client.modify_join(command.cube, command.member_name, command.fields)
        return self.client.delete_join(command.cube, command.member_name)
